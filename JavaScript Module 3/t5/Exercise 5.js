...
for (let x = 0; x < 9; x++) {
  ...
  let title = picArray[x].title
  let caption = picArray[x].caption
  let description = picArray[x].description
  let image = picArray[x].image.medium

  let h2 = document.createElement("h2")
  h2.innerText=`${title}`

  let image_card = document.createElement("figure")
  ...
  image_card.appendChild(figcaption)

  let text = document.createElement("text")
  text.innerText=`${description}`

  article.appendChild(h2)
  article.appendChild(image_card)
